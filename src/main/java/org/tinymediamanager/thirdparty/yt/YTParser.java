...
/**
 * the class {@link YTParser} implements the YT parsing
 * 
 * @author Manuel Laggner
 */
class YTParser implements Parser, TmmFeature {
  private static final Pattern SUBTITLE_LANG_CODE_REGEX = Pattern.compile("lang_code=\"(.{2,3})\"");
  ...
  public YTParser() throws ScrapeException {

    if (!isFeatureEnabled()) {
      throw new ScrapeException(new FeatureNotEnabledException(this));
    }

    extractor = new YTExtractor();
  ...
  public String getJsUrl(JSONObject config) throws YoutubeException {
    ...
    if (config.containsKey("assets")) {
      ...
      String html = extractor.loadUrl(getApiKey() + videoId);
  ...
  public List<SubtitlesInfo> getSubtitlesInfo(String videoId) throws YoutubeException {
    ...
    Matcher matcher = SUBTITLE_LANG_CODE_REGEX.matcher(subtitlesXml);
  ...
  private void populateFormats(List<Format> formats, JSONArray jsonFormats, String jsUrl, boolean isAdaptive)
      throws YoutubeException.CipherException {
  ...
  private Format parseFormat(JSONObject json, String jsUrl, boolean isAdaptive) throws YoutubeException {
    if (json.containsKey("signatureCipher")) {
      ...
      urlWithSig = URLDecoder.decode(urlWithSig, StandardCharsets.UTF_8);
  ...
  private void loadPlaylistContinuation(String continuation, List<PlaylistVideoDetails> videos, String clientVersion) throws YoutubeException {
    ...
    try {
      ...
      if (response.containsKey("continuationContents")) {
        content = response.getJSONObject("continuationContents").getJSONObject("playlistVideoListContinuation");
      }
      else {
        content = response.getJSONArray("onResponseReceivedActions").getJSONObject(0).getJSONObject("appendContinuationItemsAction");
      }
