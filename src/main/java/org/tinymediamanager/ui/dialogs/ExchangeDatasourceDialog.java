...
public class ExchangeDatasourceDialog extends TmmDialog {
  ...
  public ExchangeDatasourceDialog(String datasource) {
    super(SettingsDialog.getInstance(), BUNDLE.getString("Settings.exchangedatasource"), "exchangeDatasource");

    JPanel panelContent = new JPanel();
    getContentPane().add(panelContent, BorderLayout.CENTER);
    panelContent.setLayout(new MigLayout("", "[20lp!][400lp,grow]", "[][10lp!][][][10lp!][][][20lp!]"));
    {
      JTextPane tpDescription = new ReadOnlyTextPane(BUNDLE.getString("Settings.exchangedatasource.desc"));
      panelContent.add(tpDescription, "cell 0 0 2 1,grow");

      JLabel lblOldDatasourceT = new TmmLabel(BUNDLE.getString("Settings.exchangedatasource.old"));
      ...
      JLabel lblNewDatasourceT = new TmmLabel(BUNDLE.getString("Settings.exchangedatasource.new"));
      ...
      btnChooseNewDatasource.addActionListener(e -> {
        Path file = TmmUIHelper.selectDirectory(BUNDLE.getString("Settings.datasource.folderchooser"), datasource);
    ...
    }
    {
      ...
      okButton.addActionListener(e -> {
        if (StringUtils.isBlank(lblNewDatasource.getText())) {
          JOptionPane.showMessageDialog(ExchangeDatasourceDialog.this, BUNDLE.getString("Settings.exchangedatasource.error"),
              BUNDLE.getString("Settings.exchangedatasource"), JOptionPane.ERROR_MESSAGE);
          return;
        }
        setVisible(false);
      });
