...
/**
 * the class {@link TmmPopupFactory} is used to force heavy weight popups for all system (not only MacOS and Linux)
 *
 * @author Karl Tauber, modified by Manuel Laggner
 */
public class TmmPopupFactory extends FlatPopupFactory {
   ...
   * <p>
   * PopupFactory caches heavy weight popup windows and reuses them. On a dual screen setup, if the popup owner has moved from one screen to the other
   * one, then the cached heavy weight popup window may be connected to the wrong screen. If the two screens use different scaling factors, then the
   * popup location and size is scaled when the popup becomes visible, which shows the popup in the wrong location (or on wrong screen). The
   * re-scaling is done in WWindowPeer.setBounds() (Java 9+).
   * <p>
   ...
   * <p>
   * On a dual screen setup, where screens use different scale factors, it may happen that the window location changes when showing a heavy weight
   * popup window. E.g. when opening an dialog on the secondary screen and making combobox popup visible.
   * <p>
   * This is a workaround for https://bugs.openjdk.java.net/browse/JDK-8224608
   */
  ...
  /**
   * There is no API in Java 8 to force creation of heavy weight popups, but it is possible with reflection. Java 9 provides a new method.
   *
   * When changing FlatLaf system requirements to Java 9+, then this method can be replaced with: return getPopup( owner, contents, x, y, true );
   */
  private Popup getHeavyWeightPopup(Component owner, Component contents, int x, int y) throws IllegalArgumentException {
    try {
      if (java9getPopupMethod == null) {
        java9getPopupMethod = PopupFactory.class.getDeclaredMethod("getPopup", Component.class, Component.class, int.class, int.class, boolean.class);
      }
  ...
  /**
   * Usually ToolTipManager places a tooltip at (mouseLocation.x, mouseLocation.y + 20). In case that the tooltip would be partly outside of the
   * screen, ToolTipManagerthe changes the location so that the entire tooltip fits on screen. But this can place the tooltip under the mouse location
   * and hide the owner component.
   * <p>
   * This method checks whether the current mouse location is within tooltip bounds and corrects the y-location so that the tooltip is placed above
   * the mouse location.
   */
  ...
  private class DropShadowPopup extends NonFlashingPopup {
    ...
    private Border createDropShadowBorder() {
      return new FlatDropShadowBorder(UIManager.getColor("Popup.dropShadowColor"), UIManager.getInsets("Popup.dropShadowInsets"),
